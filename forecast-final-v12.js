...
function expertLabel(k){return({NUMERICAL:"Giá / kỹ thuật",REGIME:"Trạng thái thị trường",VOLATILITY:"Biến động thực tế",SECTOR:"Luân chuyển ngành",EVENT:"Tin tức / sự kiện",FLOW:"Dòng tiền tổ chức",FUND:"Danh mục quỹ",FUNDAMENTAL_EVENT:"Sự kiện doanh nghiệp",RUMOR:"Thông tin lan truyền"})[k]||k}
...
function renderSource(B,sym){const box=$("#sourceAudit");box.replaceChildren();const z=B.dash.symbols?.[sym]||{},u=B.model.universe||{},signal=B.market?.sources?.signalAudit||{},fundAudit=B.market?.sources?.fundAudit||{},flow=z.flow||{},fund=z.fundContext||{},rows=[["Giá thị trường",z.date||"—",z.dataFreshness==="CURRENT"?"Đã cập nhật":"Cần cập nhật"],["Độ phủ HOSE",`${u.currentSymbols??"—"} mã`,finite(u.hoseCoverage)?pct(u.hoseCoverage,1):"—"],["Tin đã đối chiếu",`${(+signal.acceptedEvents||0).toLocaleString("vi-VN")} bài`,`${signal.newsSymbols??"—"} mã cổ phiếu`],["Dòng tiền tổ chức",flow.stale?"CHƯA MỚI":(flow.foreignAvailable||flow.propAvailable?"ĐÃ CẬP NHẬT":"CHƯA CÓ"),flow.stale?`${flow.sessionsSinceObservation??"—"} phiên trước`:`Khối ngoại & tự doanh`],["Danh mục quỹ",fund.available?`${fund.fundCount||0} quỹ`:"CHƯA CÓ",fund.available?`Bình quân ${pct(fund.averageReportedWeight,1)}/quỹ · ${fund.modelEligible?"đã vào model":"đang tích lũy lịch sử"}`:`${fundAudit.snapshotCount||0} snapshot`]];for(const[label,value,detail]of rows){const e=document.createElement("article");e.className="sourceCard";e.innerHTML=`<span>${esc(label)}</span><b>${esc(value)}</b><small>${esc(detail)}</small>`;box.append(e)}}
