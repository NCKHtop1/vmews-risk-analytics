...
function sessionQuote(symbol){
  const session=window.__VMEWS_SESSION__,name=String(symbol||"").toUpperCase();
  if(!sessionUsableForDetail(session))return null;
  const quote=(session.symbols||[]).find(item=>String(item?.symbol||"").toUpperCase()===name);
  return quote?.quoteCurrent===true&&quote?.freshForCutoff!==false&&finite(quote?.liveClose)&&+quote.liveClose>0?quote:null;
}
function sessionUsableForDetail(session){
  if(session?.status!=="PASS"||session?.coreForecastUnchanged!==true||!Array.isArray(session?.symbols))return false;
  const observed=Date.parse(session.cutoffAt||session.generatedAt||"");
  return Number.isFinite(observed)&&observed<=Date.now()+300000&&Date.now()-observed<80*60*60*1000;
}
function withSessionPrice(symbol,snapshot){
  const quote=sessionQuote(symbol);
  if(!quote)return snapshot;
  return{...snapshot,close:+quote.liveClose,priceSession:{
    source:window.__VMEWS_SESSION__?.source||"SESSION_OVERLAY",
    session:window.__VMEWS_SESSION__?.session||"SESSION",
    observedAt:quote.observedAt||quote.updateAt||window.__VMEWS_SESSION__?.cutoffAt||null,
    quoteDate:String(quote.lastTradeAt||quote.updateAt||window.__VMEWS_SESSION__?.cutoffAt||"").slice(0,10),
    coreClose:+snapshot.close,change:finite(quote.change)?+quote.change:null,
  }};
}
function statusText(B,sym,z){
  if(z.priceSession){
    const date=z.priceSession.quoteDate||String(z.priceSession.observedAt||"").slice(0,10)||"phiên mới nhất";
    return`${sym} · giá ${date}: ${price(z.close)} · forecast niêm phong ${B.dash.asOf||z.date||"—"}`;
  }
  return`${sym} · giá EOD ${z.date||B.dash.asOf||"—"}: ${price(z.close)}${z.dataFreshness==="STALE_EOD"?" · cần cập nhật":""}`;
}
window.__VMEWS_WITH_SESSION_PRICE__=withSessionPrice;
...
function draw(sym,z,history,animate=true){
  ...
  const hist=(history||[]).slice(-chartRange).map(item=>({...item}));
  if(z.priceSession){const liveDate=z.priceSession.quoteDate||String(z.priceSession.observedAt||"").slice(0,10);if(liveDate&&hist.at(-1)?.date===liveDate)hist[hist.length-1]={...hist.at(-1),close:+z.close,rawClose:+z.close};else if(liveDate)hist.push({date:liveDate,close:+z.close,rawClose:+z.close,volume:null,sessionOverlay:true})}
...
function rerender(B,sym,core){const z=withSessionPrice(sym,core);last={B,sym,z,core};renderSummary(B,sym,z);renderForecastCards(z);renderDrivers(z,5);draw(sym,z,B.dash.charts?.[sym]||[]);renderNews(core);renderRumors(core);renderSource(B,sym);renderBacktest(B,sym,btH);setText("#status",statusText(B,sym,z));window.dispatchEvent(new CustomEvent("vmews:symbol-changed",{detail:{symbol:sym,snapshot:z,coreSnapshot:core}}))}
...
async function renderSymbol(raw){const B=await loadBase();assertProduction(B);const sym=String(raw||"").trim().toUpperCase(),z=B.dash.symbols?.[sym];if(!z)throw Error(`${sym}: chưa có đủ dữ liệu để phân tích.`);$("#symbol").value=sym;setText("#sparkSymbol",sym);rerender(B,sym,z);history.replaceState(null,"",`?symbol=${encodeURIComponent(sym)}`)}
window.__VMEWS_RENDER_SYMBOL__=renderSymbol;
async function init(){try{const B=await loadBase();assertProduction(B);quickButtons(B);const q=new URLSearchParams(location.search).get("symbol")||"FPT";await renderSymbol(q);bindChartHover();void refreshCommunity(B);window.setInterval(()=>{if(!document.hidden)void refreshCommunity(B)},120000);$("#go").onclick=()=>renderSymbol($("#symbol").value).catch(showError);$("#symbol").addEventListener("keydown",e=>{if(e.key==="Enter")renderSymbol(e.currentTarget.value).catch(showError)});window.addEventListener("resize",()=>{if(last)draw(last.sym,last.z,last.B.dash.charts?.[last.sym]||[])});window.addEventListener("vmews:session-updated",()=>{if(last?.core)rerender(last.B,last.sym,last.core)})}catch(e){showError(e)}}
