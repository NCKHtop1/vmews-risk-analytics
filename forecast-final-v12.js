...
function applySessionView(symbol,snapshot,session=window.__VMEWS_SESSION__){const quote=sessionQuote(symbol,session);if(!quote||+quote.liveClose<=0)return snapshot;return{...snapshot,coreClose:+snapshot.close,close:+quote.liveClose,liveSession:{session:session?.session||null,cutoffAt:session?.cutoffAt||null,updateAt:quote.updateAt||null,change:finite(quote.change)?+quote.change:null,coreAsOf:session?.coreAsOf||null}}}
