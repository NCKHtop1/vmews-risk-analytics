...
function decision(z){const q=h(z,5);if(!validatedPrice(q))return{label:"CHƯA ĐỦ DỮ LIỆU",tone:"warning",text:"Chưa thể xác định vùng giá T+5."};const er=+q.expectedReturn||0,p=validatedDirection(q)?+q.probUp:null,r=z.riskStatus||"GREEN",ps=validatedDirection(q)?` · P(tăng) ${pct(p,0)}`:"",vol=Math.max(.004,Math.min(.012,(+z.dailyVolatility||.02)*.30)),line=`Trọng tâm ${price(q.expectedPrice)} (${er>=0?"+":""}${pct(er)})${ps} · vùng giá ${price(q.q20Price)} – ${price(q.q80Price)}.`;if(r==="RED")return{label:"RỦI RO CAO",tone:"negative",text:line};if(r==="YELLOW")return{label:er>=0?"TÍCH CỰC CÓ ĐIỀU KIỆN":"THẬN TRỌNG",tone:"warning",text:line};if(er>=vol&&(!validatedDirection(q)||p>.46))return{label:"NGHIÊNG TĂNG",tone:"positive",text:line};if(er<=-vol&&(!validatedDirection(q)||p<.54))return{label:"NGHIÊNG GIẢM",tone:"negative",text:line};return{label:"TRUNG TÍNH",tone:"neutral",text:line}}

function renderDrivers(z,horizon=5){const q=h(z,horizon),box=$("#drivers");box.replaceChildren();setText("#driverTitle",`Các yếu tố chính · T+${horizon}`);setText("#expertMeta","");if(!validatedPrice(q)){box.innerHTML='<div class="empty">Chưa đủ dữ liệu để phân tích.</div>';return}const c=q.expertContributions||{},entries=Object.entries(c).sort((a,b)=>Math.abs(+b[1])-Math.abs(+a[1])),max=Math.max(...entries.map(x=>Math.abs(+x[1]||0)),1e-6);for(const[name,val]of entries){const e=document.createElement("article");e.className="driver";e.innerHTML=`<span>${esc(expertLabel(name))}</span><b class="${driverTone(+val)}">${+val>=0?"+":""}${pct(val)}</b><div class="driverBar ${driverTone(+val)}"><i style="width:${Math.min(100,Math.abs(+val)/max*100)}%"></i></div>`;box.append(e)}if(!entries.length)box.innerHTML='<div class="empty">Chưa có yếu tố đủ nổi bật.</div>'}

function renderForecastCards(z){const box=$("#forecastCards");box.replaceChildren();for(let n=1;n<=5;n++){const q=h(z,n),e=document.createElement("article");e.className="forecastCard"+(n===5?" active":"");e.setAttribute("role","button");e.tabIndex=0;e.setAttribute("aria-label",`Xem phân tích kỳ T+${n}`);if(!validatedPrice(q)){e.innerHTML=`<span>T+${n}</span><strong>Chưa đủ dữ liệu</strong>`}else{const d=(+q.expectedPrice/+z.close)-1,date=q.targetDate?new Date(`${q.targetDate}T00:00:00`).toLocaleDateString("vi-VN",{day:"2-digit",month:"2-digit"}):"";e.innerHTML=`<span>T+${n}${date?` · ${date}`:""}</span><strong>${price(q.expectedPrice)}</strong><small>Trọng tâm ${d>=0?"▲ +":"▼ "}${pct(d,2)}</small><small>Vùng giá ${price(q.q20Price)} – ${price(q.q80Price)}</small><small>P↑ ${pupText(q,0)}</small>`}const activate=()=>{document.querySelectorAll(".forecastCard").forEach(x=>x.classList.remove("active"));e.classList.add("active");renderDrivers(z,n);if(last){renderEventImpact(last.B,z,n);draw(last.sym,last.z,last.B.dash.charts?.[last.sym]||[],false)}};e.onclick=activate;e.onkeydown=event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();activate()}};box.append(e)}}
...
function renderSource(B,sym){const box=$("#sourceAudit");box.replaceChildren();const z=B.dash.symbols?.[sym]||{},u=B.model.universe||{},signal=B.market?.sources?.signalAudit||{},flow=z.flow||{},rows=[["Giá thị trường",z.date||"—",z.dataFreshness==="CURRENT"?"Đã cập nhật":"Cần cập nhật"],["Độ phủ HOSE",`${u.currentSymbols??"—"} mã`,finite(u.hoseCoverage)?pct(u.hoseCoverage,1):"—"],["Tin đã đối chiếu",`${(+signal.acceptedEvents||0).toLocaleString("vi-VN")} bài`,`${signal.newsSymbols??"—"} mã cổ phiếu`],["Dòng tiền tổ chức",flow.stale?"CHƯA MỚI":(flow.foreignAvailable||flow.propAvailable?"ĐÃ CẬP NHẬT":"CHƯA CÓ"),flow.stale?`${flow.sessionsSinceObservation??"—"} phiên trước`:`Khối ngoại & tự doanh`]];for(const[label,value,detail]of rows){const e=document.createElement("article");e.className="sourceCard";e.innerHTML=`<span>${esc(label)}</span><b>${esc(value)}</b><small>${esc(detail)}</small>`;box.append(e)}}
