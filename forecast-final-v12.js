...
function renderSource(B,sym){const box=$("#sourceAudit");box.replaceChildren();const z=B.dash.symbols?.[sym]||{},u=B.model.universe||{},signal=B.market?.sources?.signalAudit||{},fundAudit=B.market?.sources?.fundAudit||{},flow=z.flow||{},fund=z.fundContext||{},foreign=flow.foreign||{},prop=flow.proprietary||{},lastFlow=[foreign.latestDate,prop.latestDate].filter(Boolean).sort().at(-1),rows=[["Giá thị trường",z.date||"—",z.dataFreshness==="CURRENT"?"Đã cập nhật":"Cần cập nhật"],["Độ phủ HOSE",`${u.currentSymbols??"—"} mã`,finite(u.hoseCoverage)?pct(u.hoseCoverage,1):"—"],["Tin đã đối chiếu",`${(+signal.acceptedEvents||0).toLocaleString("vi-VN")} bài`,`${signal.newsSymbols??"—"} mã cổ phiếu`],["Dòng tiền tổ chức",lastFlow||"CHƯA CÓ",lastFlow?"Khối ngoại & tự doanh":"Đang cập nhật"],["Danh mục quỹ",fund.available?`${fund.fundCount||0} quỹ`:"CHƯA CÓ",fund.available?`${pct(fund.averageReportedWeight,1)}/quỹ · ${fund.usedByForecast?"đang tác động dự báo":"chờ công bố hợp lệ"}`:`${fundAudit.snapshotCount||0} kỳ công bố`],["Tài chính doanh nghiệp",z.fundamentalContext?.available?"ĐÃ TÍCH HỢP":"CHƯA CÓ",z.fundamentalContext?.available?"Kết quả kinh doanh & định giá":"Đang mở rộng nguồn"]];for(const[label,value,detail]of rows){const e=document.createElement("article");e.className="sourceCard";e.innerHTML=`<span>${esc(label)}</span><b>${esc(value)}</b><small>${esc(detail)}</small>`;box.append(e)}}
