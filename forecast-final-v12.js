...
function expertLabel(k){return({NUMERICAL:"Giá / kỹ thuật",REGIME:"Trạng thái thị trường",VOLATILITY:"Biến động thực tế",SECTOR:"Luân chuyển ngành",EVENT:"Tin tức / sự kiện",FLOW:"Dòng tiền tổ chức",FUNDAMENTAL_EVENT:"Sự kiện doanh nghiệp",RUMOR:"Thông tin lan truyền"})[k]||k}
...
function renderSource(B,sym){const box=$("#sourceAudit");box.replaceChildren();const a=B.audit.symbols?.[sym]||{},z=B.dash.symbols?.[sym]||{},u=B.model.universe||{},signal=B.market?.sources?.signalAudit||{},flow=z.flow||{},reject=signal.rejected||{},rows=[["Nguồn giá EOD",z.marketDataSource||a.route||"—",`${z.date||"—"} · ${z.dataFreshness==="CURRENT"?"cùng phiên":"cảnh báo: dữ liệu cũ"}`],["Độ phủ toàn sàn HOSE",finite(u.hoseCoverage)?pct(u.hoseCoverage,1):"—",`${u.currentSymbols??"—"}/${u.listedHOSE??"—"} mã · ${u.insufficientHistorySymbols?.length||0} mã thiếu lịch sử`],["Tin đã kiểm tra đúng mã",`${(+signal.acceptedEvents||0).toLocaleString("vi-VN")} bài`,`${signal.newsSymbols??"—"} mã · loại ${reject.issuer_mismatch||0} bài nhầm công ty`],["Dòng tiền tổ chức",flow.stale?"CHƯA CẬP NHẬT":(flow.foreignAvailable||flow.propAvailable?"ĐỦ DỮ LIỆU":"CHƯA CÓ"),flow.stale?`${flow.sessionsSinceObservation??"—"} phiên từ dữ liệu gần nhất`:`khối ngoại ${flow.foreignAvailable?"có":"không"} · tự doanh ${flow.propAvailable?"có":"không"}`],["Tin công bố sau 15:00","PHIÊN KẾ TIẾP","Không dùng thông tin chưa xuất hiện tại T0"],["BCTC / dữ liệu cơ bản","KHÔNG DÙNG NẾU THIẾU PIT","Loại bỏ nếu không xác minh được ngày công bố"],["Sai bước giá HOSE","0 BÁO GIÁ",`${u.currentSymbols??"—"} mã × 5 kỳ hạn`],["Kiểm định mô hình",B.model.promotion?.status||"—",`${B.model.featureNames?.length||0} biến · 4 giai đoạn OOS`]];for(const[label,value,detail]of rows){const e=document.createElement("article");e.className="sourceCard";e.innerHTML=`<span>${esc(label)}</span><b>${esc(value)}</b><small>${esc(detail)}</small>`;box.append(e)}}

function renderEventImpact(B,z,horizon=5){const box=$("#eventImpact");if(!box)return;const hAudit=B.model.horizons?.[String(horizon)]||{},study=hAudit.eventImpactAudit||{},news=study.positiveNews||{},negative=study.negativeNews||{},ab=hAudit.factorAblation||{},feature=z.newsFeatures||{},signed=v=>finite(v)?`${+v>=0?"+":""}${pct(v,2)}`:"—";setText("#eventImpactMeta",`T+${horizon} · ${(study.observations||0).toLocaleString("vi-VN")} phiên có tin trên holdout độc lập · cắt dữ liệu 15:00`);box.innerHTML=`<article class="proofCard"><span>Tin tích cực → giá thực tế</span><b class="${(+news.meanAbnormalReturn)>=0?"good":"bad"}">${signed(news.meanAbnormalReturn)}</b><small>lợi suất vượt thị trường · ${news.n||0} quan sát</small></article><article class="proofCard"><span>Tin tiêu cực → giá thực tế</span><b class="${(+negative.meanAbnormalReturn)>=0?"good":"bad"}">${signed(negative.meanAbnormalReturn)}</b><small>lợi suất vượt thị trường · ${negative.n||0} quan sát</small></article><article class="proofCard"><span>Giá trị bổ sung của news</span><b class="${(+ab.EVENT?.deltaMAEImprove)>=0?"good":"bad"}">${signed(ab.EVENT?.deltaMAEImprove)}</b><small>Δ MAE skill khi loại nhóm tin khỏi mô hình</small></article><article class="proofCard"><span>Tin của ${esc(z.symbol||"")} · 5 phiên</span><b>${feature.count5||0} bài</b><small>${feature.positive5||0} tích cực · ${feature.negative5||0} tiêu cực · ${feature.official5||0} chính thức</small></article>`}
...
async function renderSymbol(raw){const B=await loadBase();assertProduction(B);const sym=String(raw||"").trim().toUpperCase(),z=B.dash.symbols?.[sym];if(!z)throw Error(`${sym}: chưa có đủ lịch sử HOSE để dự báo.`);rerender(B,sym,z);setText("#status",`${sym} · EOD ${z.date||B.dash.asOf||"—"} · ${z.dataFreshness==="STALE_EOD"?"CẢNH BÁO: giá chưa cập nhật":"mô hình đã kiểm định"}`);history.replaceState(null,"",`?symbol=${encodeURIComponent(sym)}`)}
