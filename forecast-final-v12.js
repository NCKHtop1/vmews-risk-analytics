...
function forecastAvailable(q){return finite(q?.expectedPrice)&&finite(q?.q20Price)&&finite(q?.q80Price)}
...
function sessionQuote(symbol,session=window.__VMEWS_SESSION__){return(session?.symbols||[]).find(item=>item.symbol===symbol&&item.quoteCurrent&&item.freshForCutoff!==false&&finite(item.liveClose))||null}
...
function decision(z){const q=h(z,5);if(!forecastAvailable(q))return{label:"CHƯA ĐỦ DỮ LIỆU",tone:"warning",text:"Chưa thể xác định phân phối giá T+5."};if(z.liveSession?.forecastAligned===false)return{label:"GIÁ MỚI · FORECAST CHỜ CẬP NHẬT",tone:"warning",text:`Giá xác thực mới nhất là ${price(z.close)}, nhưng forecast lõi vẫn ở ${z.liveSession.coreAsOf||"phiên cũ"}; hệ thống tạm ngừng kết luận và xếp hạng đến khi lõi cập nhật ${z.liveSession.expectedCoreAsOf||"phiên hoàn tất mới nhất"}.`};const er=finite(z.close)?+q.expectedPrice/+z.close-1:+q.expectedReturn||0,p=validatedDirection(q)?+q.probUp:null,r=z.riskStatus||"GREEN",ps=validatedDirection(q)?` · P(tăng) ${pct(p,0)}`:"",move=finite(q.expectedAbsReturn)?` · biên độ hai chiều ±${pct(q.expectedAbsReturn,2)}`:"",vol=Math.max(.004,Math.min(.012,(+z.dailyVolatility||.02)*.30)),basis=z.liveSession?"so với giá phiên":"so với T0",line=`Vùng Q20–Q80 ${price(q.q20Price)} – ${price(q.q80Price)} · trung vị có điều kiện ${price(q.expectedPrice)} (${er>=0?"+":""}${pct(er)} ${basis})${ps}${move}.`;if(!validatedPrice(q)||!reliablePoint(q))return{label:"PHÂN PHỐI THỬ NGHIỆM · KHÔNG PHẢI TÍN HIỆU",tone:"warning",text:`${line} Điểm giữa chưa chứng minh được độ chính xác kinh tế ổn định; xem vùng giá và hai kịch bản, không suy diễn mức tăng/giảm vài trăm đồng là mục tiêu chắc chắn.`};if(r==="RED")return{label:"RỦI RO CAO",tone:"negative",text:line};if(r==="YELLOW")return{label:er>=0?"TÍCH CỰC CÓ ĐIỀU KIỆN":"THẬN TRỌNG",tone:"warning",text:line};if(q.conditionalValueValidated===false)return{label:er>=vol?"NGHIÊNG TĂNG · CHỈ THEO DÕI":er<=-vol?"NGHIÊNG GIẢM · CHỈ THEO DÕI":"THEO DÕI · CHƯA CÓ LỢI THẾ SAU PHÍ",tone:"warning",text:`${line} Chưa chứng minh được lợi thế giao dịch sau chi phí.`};if(er>=vol&&(!validatedDirection(q)||p>.46))return{label:"NGHIÊNG TĂNG",tone:"positive",text:line};if(er<=-vol&&(!validatedDirection(q)||p<.54))return{label:"NGHIÊNG GIẢM",tone:"negative",text:line};return{label:"TRUNG TÍNH",tone:"neutral",text:line}}

function renderDrivers(z,horizon=5){const q=h(z,horizon),box=$("#drivers");box.replaceChildren();setText("#driverTitle",`Các yếu tố chính · T+${horizon}`);setText("#expertMeta",forecastAvailable(q)&&!validatedPrice(q)?"Ước lượng độ tin cậy thấp":"");if(!forecastAvailable(q)){box.innerHTML='<div class="empty">Chưa đủ dữ liệu để phân tích.</div>';return}const c=q.expertContributions||{},entries=Object.entries(c).filter(([,value])=>Math.abs(+value||0)>1e-7).sort((a,b)=>Math.abs(+b[1])-Math.abs(+a[1])),max=Math.max(...entries.map(x=>Math.abs(+x[1]||0)),1e-6);for(const[name,val]of entries){const e=document.createElement("article");e.className="driver";e.innerHTML=`<span>${esc(expertLabel(name))}</span><b class="${driverTone(+val)}">${+val>=0?"+":""}${pct(val)}</b><div class="driverBar ${driverTone(+val)}"><i style="width:${Math.min(100,Math.abs(+val)/max*100)}%"></i></div>`;box.append(e)}if(!entries.length)box.innerHTML='<div class="empty">Chưa có yếu tố đủ nổi bật.</div>'}

function renderForecastCards(z){const box=$("#forecastCards");box.replaceChildren();for(let n=1;n<=5;n++){const q=h(z,n),e=document.createElement("article");e.className="forecastCard"+(n===5?" active":"");e.setAttribute("role","button");e.tabIndex=0;e.setAttribute("aria-label",`Xem phân tích kỳ T+${n}`);if(!forecastAvailable(q)){e.innerHTML=`<span>T+${n}</span><strong>CHƯA CÓ DỮ LIỆU</strong><small>Không đủ đầu vào để ước lượng</small>`}else{const d=(+q.expectedPrice/+z.close)-1,date=q.targetDate?new Date(`${q.targetDate}T00:00:00`).toLocaleDateString("vi-VN",{day:"2-digit",month:"2-digit"}):"",confidence=validatedPrice(q)&&reliablePoint(q)?"ĐIỂM ĐÃ QUA GATE":"ĐỘ TIN CẬY ĐIỂM THẤP",scenario=finite(q.expectedAbsReturn)&&finite(q.bearScenarioPrice)&&finite(q.bullScenarioPrice)?`<small>Biên độ học được ±${pct(q.expectedAbsReturn,2)}</small><small>Kịch bản giảm/tăng ${price(q.bearScenarioPrice)} / ${price(q.bullScenarioPrice)}</small>`:"";e.innerHTML=`<span>T+${n}${date?` · ${date}`:""} · ${confidence}</span><strong>${price(q.q20Price)} – ${price(q.q80Price)}</strong><small>Vùng Q20–Q80 · không phải chênh vài trăm đồng</small><small>Trung vị ${price(q.expectedPrice)} (${d>=0?"+":""}${pct(d,2)})</small>${scenario}<small>${pupText(q,0)}</small>`}const activate=()=>{document.querySelectorAll(".forecastCard").forEach(x=>x.classList.remove("active"));e.classList.add("active");renderDrivers(z,n);if(last){renderEventImpact(last.B,z,n);draw(last.sym,last.z,last.B.dash.charts?.[last.sym]||[],false)}};e.onclick=activate;e.onkeydown=event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();activate()}};box.append(e)}}
...
function renderSummary(B,sym,z){
  const d=decision(z),de=$("#decision");de.textContent=d.label;de.className=`decision ${d.tone}`;setText("#summary",d.text);setText("#close",price(z.close));
  for(const n of [1,3,5]){const q=h(z,n);setText(`#t${n}`,forecastAvailable(q)?`${price(q.q20Price)} – ${price(q.q80Price)}`:"—")}
  const q5=h(z,5),audit=B.model.horizons?.["5"]?.sealedAudit||{},ratio=finite(q5.magnitudeCalibrationRatio)?+q5.magnitudeCalibrationRatio:finite(audit.medianExpectedAbsMove)&&finite(audit.realizedMedianAbs)?+audit.medianExpectedAbsMove/Math.max(+audit.realizedMedianAbs,1e-12):null,signValidated=q5.pointDirectionValidated===true||(+audit.directionalAccuracy||0)>=.52&&(audit.chronologicalFolds||[]).filter(item=>(+item.directionalAccuracy||0)>.5).length>=3,signRate=finite(q5.historicalDirectionAccuracy)?+q5.historicalDirectionAccuracy:+audit.directionalAccuracy,rank=q5.crossSectionalRankValidated===true&&finite(q5.crossSectionalRankPercentile)?` ${sym} thuộc nhóm ${Math.max(1,Math.ceil((1-+q5.crossSectionalRankPercentile)*100))}% có xếp hạng mạnh nhất toàn HOSE.`:"",liveScenario=q5.liveScenarioOverlay?.scenarioPrice?" Thông tin mới chỉ được lưu thành kịch bản tham khảo và không làm thay đổi giá trung tâm.":"",position=z.liveSession?.forecastAligned===false?"":sessionPosition(q5,z.close),livePosition=position==="ABOVE_BULL"?` Giá phiên ${price(z.close)} đã vượt cả kịch bản tăng ${price(q5.bullScenarioPrice)}; đây là cảnh báo vượt mô hình cũ, không phải forecast mới.`:position==="ABOVE_Q80"?` Giá phiên ${price(z.close)} đã vượt Q80 ${price(q5.q80Price)}; cần định vị lại trước khi ra quyết định.`:position==="BELOW_BEAR"?` Giá phiên ${price(z.close)} đã thấp hơn kịch bản giảm ${price(q5.bearScenarioPrice)}; rủi ro đã vượt mô hình cũ.`:position==="BELOW_Q20"?` Giá phiên ${price(z.close)} đã thấp hơn Q20 ${price(q5.q20Price)}.`:"";
  setText("#range5",forecastAvailable(q5)?price(q5.expectedPrice):"—");setText("#pup",validatedDirection(q5)?`P(tăng) ${pct(q5.probUp,0)}`:signValidated?`Đúng chiều lịch sử ${pct(signRate,0)}`:"CHƯA ĐỦ KIỂM ĐỊNH");setText("#move5",finite(q5.expectedAbsReturn)?`±${pct(q5.expectedAbsReturn,2)}`:"—");setText("#bear5",finite(q5.bearScenarioPrice)?price(q5.bearScenarioPrice):"—");setText("#scenarioCenter5",forecastAvailable(q5)?price(q5.expectedPrice):"—");setText("#bull5",finite(q5.bullScenarioPrice)?price(q5.bullScenarioPrice):"—");
  const pointWarning=reliablePoint(q5)?"":"Điểm giữa hiện có độ tin cậy kinh tế thấp; vùng giá và kịch bản là phần trình bày chính. ";setText("#scenarioCaveat",`${pointWarning}${finite(ratio)?`Biên độ mô hình bám ${pct(ratio,0)} mức biến động ngoài mẫu. `:""}${validatedDirection(q5)?"Xác suất chiều giá đã qua kiểm định; kịch bản vẫn không phải cam kết giá.":signValidated?`Dấu dự báo đúng ${pct(signRate,1)} ngoài mẫu; đây không phải xác suất tăng của mã. Hai kịch bản vẫn có thể xảy ra.`:"Chiều tăng/giảm T+5 chưa qua kiểm định xác suất; hai kịch bản chỉ biểu diễn độ lớn có thể xảy ra."}${livePosition}${liveScenario}${rank}`);setText("#risk",({GREEN:"THẤP",YELLOW:"THEO DÕI",WATCH:"THEO DÕI",RED:"CAO"})[z.riskStatus]||"—");setText("#chartTitle",`${sym} · ${price(z.close)} → T+1…T+5`);const modelState=reliablePoint(q5)?"ĐIỂM QUA GATE":"PHÂN PHỐI · ĐIỂM ĐỘ TIN CẬY THẤP";setText("#modelBadge",`HOSE · ${B.dash.asOf||z.date||"—"} · ${modelState}${z.liveSession?` · ${z.liveSession.session||"PHIÊN"}`:""}${z.liveSession?.forecastAligned===false?" · FORECAST CHỜ CẬP NHẬT":""}`)
}
...
function rerender(B,sym,z){const view=applySessionView(sym,z);last={B,sym,z,view};renderSummary(B,sym,view);renderForecastCards(view);renderDrivers(view,5);draw(sym,view,B.dash.charts?.[sym]||[]);renderNews(view);renderRumors(view);renderSource(B,sym);renderBacktest(B,sym,btH);window.dispatchEvent(new CustomEvent("vmews:symbol-changed",{detail:{symbol:sym,snapshot:view}}))}
