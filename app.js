...
const INFO={
  liveQuote:['Live / latest quote','The live quote is fetched separately from the completed daily-bar model. During a trading session it can be newer than the model-as-of date. The displayed change is recomputed against the previous completed model close so source-specific percentage formats cannot distort the move.'],
  ...
  modelAsOf:['Model as-of date','The latest completed daily observation used by the EWS. During the Vietnam trading session, a same-day incomplete OHLCV bar is excluded from the daily model and the separate live quote continues to refresh. After the close/publication buffer, the completed daily bar can enter the model.'],
  ...
  method:['Data and model governance','The live pipeline requests the maximum Vnstock Community daily-history window, independently refreshes a recent tail, merges by trading date, and conditionally recovers the history in blocks if the long request looks truncated. A current intraday daily bar is excluded until the close/publication buffer. Data freshness and request audit are returned with every full model run.']
};
...
function renderHeader(){
  ...
  $('riskRing').style.setProperty('--risk-angle',`${clamp(risk.score||0)*3.6}deg`);$('riskRing').dataset.state=stateClass(risk.state);renderQuote();
}
...
function renderQuality(){const d=model.dataQuality||{};const requestOk=(d.requestAudit||[]).filter(x=>x.ok).length,requestTotal=(d.requestAudit||[]).length;const vals=[['Status',d.status],['Observations',fmt.format(d.rows||0)],['Coverage',`${d.start||'—'} → ${d.end||'—'}`],['Coverage years',Number.isFinite(Number(d.coverageYears))?Number(d.coverageYears).toFixed(2):'—'],['Stale trading days (approx.)',d.staleTradingDaysApprox],['Stale calendar days',d.staleCalendarDays],['Intraday bar excluded',d.intradayBarExcluded?'YES':'NO'],['Duplicates removed',d.duplicatesRemoved],['Invalid rows removed',d.invalidRemoved],['Large gaps',d.largeGaps],['History requests',`${requestOk}/${requestTotal} successful`],['Fetch strategy',d.fetchStrategy||'—']];$('qualityMetrics').innerHTML=vals.map(([k,v])=>`<div><span>${k}</span><b>${v??'—'}</b></div>`).join('')}
