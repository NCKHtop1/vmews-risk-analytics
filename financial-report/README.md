...
Open `frontend/index.html` through an HTML-capable CDN or any static server. jsDelivr delivers source files, not an HTML application host. No API key is sent to the browser.
...
The initial release includes real MBB, FPT and VCB datasets. `data/companies.json` is the broader listing catalog, not a claim that every company already has a report. The live `financial-report-data` branch expands coverage and retains valid snapshots when a provider is unavailable. The scheduled refresh runs every four hours, with a 45-minute budget; the next run continues uncollected companies. A company without a published dataset is shown as unavailable, never given invented date limits.
