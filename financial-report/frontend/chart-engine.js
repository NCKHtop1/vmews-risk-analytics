...
class ChartController{
 ...
 async config(){try{const r=await fetch(configURL,{cache:'no-cache'});if(!r.ok)throw Error();this.options=await r.json();}catch{this.options={};}this.connect();}
 ...
 $('chart-fullscreen').addEventListener('click',async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else await $('chart-terminal').requestFullscreen();}catch{this.status('Trình duyệt không hỗ trợ toàn màn hình.');}});
 ...
 try{let data=this.cache.get(key);if(force||!data||Date.now()-data.cachedAt>300000){const url=this.options?.historyBase?new URL(`history?symbol=${symbol}&interval=${baseTF}`,this.options.historyBase).href:this.base+(baseTF==='1m'?'intraday/':'history/')+symbol+'.json';const r=await fetch(url,{signal:controller.signal,cache:'no-cache'});if(!r.ok)throw Error('HTTP '+r.status);const raw=await r.json();if(raw.symbol!==symbol||raw.unit!=='VND'||!Array.isArray(raw.bars))throw Error('Invalid history');data={...raw,bars:M.cleanBars(raw.bars,baseTF==='1m'),cachedAt:Date.now()};if(!data.bars.length)throw Error('Empty');this.cache.set(key,data);if(this.cache.size>24)this.cache.delete(this.cache.keys().next().value);}
 ...
 }catch(e){if(generation!==this.generation||e.name==='AbortError')return;this.loading=false;$('chart-status').textContent=baseTF==='1m'?'Chưa có lịch sử phút cho mã này. Chọn Ngày/Tuần/Tháng hoặc thử lại bằng Cập nhật.':'Không tải được lịch sử. Bấm Cập nhật để thử lại.';}
 }
