...
def snapshot():
 ...
 if len(pred)<30:raise RuntimeError(f'Forecast archive coverage too small: {len(pred)}')
 core={'version':VER,'asOf':d,'modelVersion':model['version'],'marketScanVersion':scan.get('version'),'sentimentVersion':sent.get('version'),'symbols':len(pred),'predictions':pred,'governance':{'futureLabelsPresent':False,'exactPriceTarget':False,'automaticPromotion':False,'sentimentNumericalFeature':False,'foreignFlowNumericalFeature':False}}
...
def manifest():
 a=[]
 for p in sorted(SNAPS.glob('????-??-??.json')):
  z=load(p);a.append({'date':z.get('asOf'),'file':str(p.relative_to(ROOT)).replace('\\','/'),'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'snapshotHash':z.get('snapshotHash'),'modelVersion':z.get('modelVersion'),'symbols':z.get('symbols')})
...
def live():
 z=snapshot();st=archive(z);ev=evaluate();dump(LIVE/'evaluation.json',ev);dump(LIVE/'manifest.json',manifest());q={'version':VER,'generatedAt':datetime.now(timezone.utc).isoformat(),'status':'PASS','asOf':z['asOf'],'archiveStatus':st,'integrity':verify(),'modelVersion':z['modelVersion'],'sentimentVersion':z.get('sentimentVersion'),'timeBasis':'COMPLETED_EOD_ONLY'};dump(LIVE/'integrity.json',q);print(json.dumps({'integrity':q,'evaluation':ev['summary']},ensure_ascii=False,indent=2))
