...
# Model architecture, point-in-time split and sealed evidence.
...
for h in range(1,6):
    ...
    ck(f'T+{h} calibration has >=7 buckets',len(z.get('calibration',[]))>=7,len(z.get('calibration',[])))
    ...
    if h>=2:
        ...
        ck(f'T+{h} probability calibration bounded',fin(a['ece']) and a['ece']<.10,a['ece'])
    else:
        ck('T+1 not falsely promoted as direction signal',not g.get('direction',False) or (a['balancedAccuracy']>.515 and a['mcc']>.02),g)

# Current inference: all displayed scenarios finite and market psychology attached.
...
# News crawler, supervised sentiment, unsupervised clusters and event-study pooling.
ns=news['summary'];ck('news corpus >=12k',ns['articles']>=12000,ns);ck('news symbols >=375',ns['symbolsWithNews']>=375,ns);ck('news median >=12 per symbol',ns['medianPerSymbol']>=12,ns);ck('news 10+ symbols >=220',ns['symbols10plus']>=220,ns)
ck('sentiment article parity',sent['summary']['articles']>=int(.98*ns['articles']),sent['summary']);ck('sentiment all symbols',sent['summary']['symbols']>=380,sent['summary']);ck('sentiment POS volume',sent['summary']['positive']>=1000);ck('sentiment NEG volume',sent['summary']['negative']>=500);ck('sentiment NEU volume',sent['summary']['neutral']>=3000)
...
ck('mainstream news present','MAINSTREAM' in source,source);ck('rumor separated','RUMOR_UNVERIFIED' in source and 'RUMOR' in streams,(source,streams));ck('disclosure-search stream present','OFFICIAL' in streams,streams);ck('publisher diversity >=100',len(pubs)>=100,len(pubs));ck('event taxonomy >=8',len(events)>=8,events);ck('PhoBERT actually used','phobert' in methods,methods)
ck('event study >=12k events',ev['events']>=12000,ev['events']);ck('event-study symbols >=375',len(ev['symbols'])>=375,len(ev['symbols']));ck('unsupervised clusters >=18',len(ev['clusters'])>=18,len(ev['clusters']));ck('rumor study >=20 events',ev['rumorStudy']['events']>=20,ev['rumorStudy']['events']);ck('T+2 is outcome only','outcomes are labels only' in ev.get('pointInTime',''),ev.get('pointInTime'));ck('hierarchy groups exist',all(k in ev['groups'] for k in ['eventLabel','cluster','sectorEvent','sectorEventLabel']),ev['groups'].keys());ck('sector hierarchy material coverage',ev.get('sectorCoverage',0)>=200,ev.get('sectorCoverage'))
pooled=sum(1 for z in ev['symbols'].values() if z.get('pooledLatest') and z['pooledLatest'].get('n',0)>=20);ck('hierarchical pooling covers >=370 symbols',pooled>=370,pooled)

# Foreign/proprietary flow. Degenerate sources must be hidden rather than presented as evidence.
fs=flow['summary'];ck('flow universe broad',fs['symbolsWithFlow']>=380,fs);ck('flow current broad',len(flow.get('current',{}))>=380,len(flow.get('current',{})));ck('foreign median depth >=60',fs.get('foreignMedianRows',0)>=60,fs);ck('flow study observations >=15k',fst.get('sampledObservations',0)>=15000,fst.get('sampledObservations'))
q=fst.get('typeQuality',{});fq=q.get('foreign',{});pq=q.get('prop',{});ck('foreign flow usable',fq.get('usable') is True,fq);ck('foreign non-degenerate',fq.get('nonzeroShare',0)>=.01,fq);ck('foreign has >=3 adequate states',fq.get('adequateStates',0)>=3,fq)
if pq.get('usable'):
    ck('proprietary flow non-degenerate',pq.get('nonzeroShare',0)>=.01,pq);ck('proprietary >=3 adequate states',pq.get('adequateStates',0)>=3,pq)
else:
    ck('degenerate proprietary layer suppressed',all('prop' not in z for z in fst.get('current',{}).values()),pq)

# Macro state is backtested with a strict prior-date rule and resilient market benchmark.
ck('macro observations >=1000',macro.get('observations',0)>=1000,macro.get('observations'));ck('macro PIT strict','strictly earlier than T' in macro.get('pointInTimeRule',''),macro.get('pointInTimeRule'));ck('macro benchmark declared',macro.get('benchmarkSource') in {'YAHOO_VNINDEX','YAHOO_VNINDEX_VN','HOSE_EQUAL_WEIGHT_MEDIAN_RETURN'},macro.get('benchmarkSource'));ck('macro current state',macro.get('current',{}).get('state') in {'SUPPORTIVE','NEUTRAL','STRESS'},macro.get('current'))
...
# Canonical risk lists and liquidity policy.
...
# Immutable live monitor and UI contract.
...
# Serialization integrity: no silent NaN/Infinity is allowed in production evidence.
...
failed=[x for x in checks if not x[1]];report={'version':'VMEWS-V11-FINAL-GATE-1.0.0','tests':len(checks),'passed':len(checks)-len(failed),'failed':len(failed),'failures':[{'name':n,'detail':d} for n,_,d in failed]};(R/'data/forecast-v11-final-gate.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(report,ensure_ascii=False));sys.exit(1 if failed else 0)
