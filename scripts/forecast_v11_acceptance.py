...
# News coverage and supervised/unsupervised evidence
...
sourceClasses=set();streams=set();events=set();methods=set();publishers=set()
for z in sent['symbols'].values():
    for x in z['items']:
        sourceClasses.add(x.get('sourceClass'));streams.add(x.get('stream'));events.add(x.get('event'));methods.add(x.get('method'));publishers.add(x.get('publisher'))
ck('mainstream verified stream present','MAINSTREAM' in sourceClasses,sourceClasses);ck('rumor stream present','RUMOR' in streams and 'RUMOR_UNVERIFIED' in sourceClasses,(streams,sourceClasses));ck('main disclosure-search stream present','MAIN' in streams and 'OFFICIAL' in streams,streams);ck('publisher diversity',len(publishers)>=20,len(publishers));ck('event taxonomy broad',len(events)>=6,events);ck('DL sentiment actually used','phobert' in methods,methods)
ck('event study large',ev['events']>=4000,ev['events']);ck('event study broad symbols',len(ev['symbols'])>=340,len(ev['symbols']));ck('unsupervised clusters',len(ev['clusters'])>=16,len(ev['clusters']));ck('rumor study exists',ev['rumorStudy']['events']>=20,ev['rumorStudy']['events']);ck('event hierarchy exists',all(k in ev['groups'] for k in ['eventLabel','cluster','sectorEvent','sectorEventLabel']),ev['groups'].keys());ck('T+2 confirmation declared','outcomes are labels only' in ev.get('pointInTime',''),ev.get('pointInTime'));ck('sector event coverage',ev.get('sectorCoverage',0)>=250,ev.get('sectorCoverage'))
pooled=sum(1 for z in ev['symbols'].values() if z.get('pooledLatest') and z['pooledLatest'].get('n',0)>=8);ck('hierarchical pooled event coverage',pooled>=250,pooled)
# Trading-flow coverage
...
# UI contract and no dead placeholders
for bad in ['Đọc hướng đi ngắn hạn','Hệ thống tách riêng','Mô hình chỉ vẽ','CHƯA CÓ','MẪU MỎNG','không phải giá mục tiêu','PhoBERT + finance rules','chưa khả dụng']:
...
ck('>=100 assertions executed before final gate',len(checks)>=100,len(checks));failed=[x for x in checks if not x[1]];report={'version':'VMEWS-V11-ACCEPTANCE-1.1.0','tests':len(checks),'passed':len(checks)-len(failed),'failed':len(failed),'failures':[{'name':a,'detail':d} for a,_,d in failed]};(ROOT/'data/forecast-v11-acceptance.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(report,ensure_ascii=False));sys.exit(1 if failed else 0)