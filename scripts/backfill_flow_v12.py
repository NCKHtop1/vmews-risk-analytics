...
ROOT=pathlib.Path(os.environ.get('GITHUB_WORKSPACE','.')).resolve();DATA=ROOT/'data';VERSION='VMEWS-FLOW-12.2.0';VN_TZ=timezone(timedelta(hours=7))
MAX_WORKERS=int(os.environ.get('V12_FLOW_WORKERS','10'));START_YEAR=int(os.environ.get('V12_FLOW_START_YEAR','2023'))
BASE=src.BASE
...
def qwindows(today):
    ...
    for y in range(START_YEAR,today.year+1):
        for sm,sd,em,ed in [(1,1,3,31),(4,1,6,30),(7,1,9,30),(10,1,12,31)]:
            a=date(y,sm,sd)
            if a>today:continue
            out.append((a,min(date(y,em,ed),today)))
    return out
...
def parse_payload(p,kind):
    a=p.get('Data') or p.get('data') or p.get('DataRows') or [];meta=p
    if isinstance(a,dict):meta={**p,**a};a=a.get('Data') or a.get('Rows') or a.get('rows') or []
    ...
    for r in a:
        ...
        if not d:continue
        if kind=='foreign':
            z={'date':d,'foreignBuyValue':nval(kk,('mua','giatri'),('mua','gt'),('buy','value')),'foreignSellValue':nval(kk,('ban','giatri'),('ban','gt'),('sell','value')),'foreignNetValue':nval(kk,('rong','giatri'),('rong','gt'),('net','value'))}
        else:
            z={'date':d,'propBuyValue':nval(kk,('mua','giatri'),('mua','gt'),('buy','value')),'propSellValue':nval(kk,('ban','giatri'),('ban','gt'),('sell','value')),'propNetValue':nval(kk,('rong','giatri'),('rong','gt'),('net','value'))}
        out.append(z)
    return out,meta
...
def foreign_rows(symbol,today):
    out=[];windows=0;empty=0
    for a,b in qwindows(today):
        r=json_window(symbol,'foreign',a,b);out.extend(r);windows+=1;empty+=not bool(r)
    return sorted({x['date']:x for x in out}.values(),key=lambda x:x['date']),{'method':'QUARTERLY_JSON_MMDDYYYY','windows':windows,'emptyWindows':empty,'startYear':START_YEAR}

def prop_rows(symbol,today):
    # CafeF proprietary export is independently audited below; no forward filling is allowed.
    r=src.export_rows(symbol,'prop')
    return [{k:v for k,v in x.items() if k in {'date','propBuyValue','propSellValue','propNetValue'}} for x in r],{'method':'FULL_RANGE_EXPORT','requestedStart':'2018-01-01','requestedEnd':today.isoformat()}
...
def one(symbol,today):
    ...
    clean,a=audit_symbol([d[k] for k in sorted(d)],{'foreign':fa,'prop':pa});return symbol,clean,a
...
def main():
    ...
    out={'version':VERSION,'generatedAt':datetime.now(timezone.utc).isoformat(),'range':{'foreignStart':f'{START_YEAR}-01-01','propRequestedStart':'2018-01-01','end':today_iso},'availabilityPolicy':'Completed-EOD only. Observation dated T is eligible for a forecast produced after the close of session T; no forward filling before first genuine observation.','source':'CafeF historical foreign quarterly JSON + proprietary full-range XLSX','sourceRole':'PIT EOD institutional-flow archive. VNStock remains primary OHLCV; community VNStock flow methods were unavailable on the production runner, so flow source is separately audited and missingness is explicit.','symbols':store,'summary':summary,'sourceAudit':aud,'failures':fail}
    audit={'version':'VMEWS-FLOW-AUDIT-12.2.0','generatedAt':out['generatedAt'],'summary':summary,'source':out['source'],'availabilityPolicy':out['availabilityPolicy'],'symbols':aud,'failures':fail}
