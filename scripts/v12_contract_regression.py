...
# Mandatory single-pass regressions for the two exact blockers that previously escaped the fast
# gate: point-family selection/evaluation mismatch and isolated-horizon deterministic assembly.
subprocess.run([sys.executable,'scripts/v12_point_selector_unit.py'],check=True)
subprocess.run([sys.executable,'scripts/v12_merge_horizon_partials_test.py'],check=True)
print('V12 ACCEPTANCE + EMBARGO + POINT SELECTOR V2 + HORIZON MERGER REGRESSION PASS')
