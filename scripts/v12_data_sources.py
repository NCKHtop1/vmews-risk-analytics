...
def vnstock_history(symbol,years=8):
    today=datetime.now(VN_TZ).date(); start=(today-timedelta(days=366*years+30)).isoformat(); end=(today+timedelta(days=1)).isoformat(); errors=[]
    _throttle_vnstock()
    try:
        ...
        if len(rows)>=MIN_ROWS:return rows,{"source":"VNSTOCK","provider":"Unified Market equity OHLCV","rows":len(rows),"unitNormalization":"x1000_to_VND" if scale==1000.0 else "VND"}
    ...
    try:
        ...
        for source in ("VCI","KBS"):
            try:
                stock=Vnstock().stock(symbol=symbol,source=source); df=stock.quote.history(start=start,end=end,interval="1D"); rows,scale=_normalize_df(df,symbol,f"Vnstock {source}")
                if len(rows)>=MIN_ROWS:return rows,{"source":"VNSTOCK","provider":f"{source} Quote.history","rows":len(rows),"unitNormalization":"x1000_to_VND" if scale==1000.0 else "VND"}
                errors.append(f"{source} only {len(rows)} rows")
            except BaseException as exc:errors.append(f"{source}: {type(exc).__name__}: {exc}")
    except BaseException as exc:errors.append(f"Vnstock import: {type(exc).__name__}: {exc}")
...
def build_price_store(symbols):
    ...
    for i,symbol in enumerate(symbols,1):
        ...
        if i%25==0 or i==len(symbols):print(json.dumps({"v12PriceProgress":i,"total":len(symbols),"passed":len(store),"failed":len(failures),"vnstockPrimary":sum(a.get("route")=="VNSTOCK_PRIMARY" for a in audits.values()),"yahooFallback":sum(a.get("route")=="YAHOO_ADJUSTED_FALLBACK" for a in audits.values())},ensure_ascii=False),flush=True)
...
def source_audit_summary(audits,failures):
    ...
    return {"version":"VMEWS-DATA-AUDIT-12.0.0","generatedAt":datetime.now(timezone.utc).isoformat(),"policy":["VNStock is the primary Vietnamese-equity OHLCV source.","Yahoo adjusted data is used as corporate-action reference and fallback.","A previous validated cache is last-resort only.","No synthetic history padding is allowed."],"symbolsPassed":len(audits),"symbolsFailed":len(failures),"routes":routes,"crossSourceMedianReturnMAD":statistics.median(mad) if mad else None,"failures":failures,"symbols":audits}
