...
for h in range(1,6):
    ...
    checks={
        ...
        'walkForwardNoFutureExpertTraining':int(wf.get('futureRowsUsedForTraining') or -1)==0,
        'walkForwardNoFutureMetaTraining':int(wf.get('futureMetaRowsUsedForTraining') or -1)==0 and str(wf.get('metaTrainMaxMaturity') or '')<str(wf.get('sealedAuditStart') or ''),
        'walkForwardNoFutureCalibration':int(wf.get('futureCalibrationRowsUsedForTraining') or -1)==0 and str(wf.get('calibrationMaxMaturity') or '')<str(wf.get('sealedAuditStart') or ''),
