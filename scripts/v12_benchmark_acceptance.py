...
matured5=[r for r in records if (r.get('priceAfter') or {}).get('5') is not None];bench5=[r for r in matured5 if (r.get('benchmarkAvailable') or {}).get('5') is True];semantic_failures=[];daily_failures=[];target_failures=[];pre_failures=[]
for r in records:
    ...
    for h in map(str,range(1,6)):
        ...
        if mature.get(h) is not None and bt.get(h)!=mature.get(h):target_failures.append((r.get('newsId'),h,bt.get(h),mature.get(h)))
        if available:
            if not finite(benchmark_value):semantic_failures.append((r.get('newsId'),h,'available_without_benchmark_value'))
            if abnormal is not None and not finite(abnormal):semantic_failures.append((r.get('newsId'),h,'nonfinite_abnormal_return'))
        elif benchmark_value is not None or abnormal is not None:semantic_failures.append((r.get('newsId'),h,'raw_or_other_return_relabelled_as_abnormal'))
        cur=car.get(h);prev=0.0 if h=='1' else car.get(str(int(h)-1));daily=dar.get(h)
        if finite(cur) and finite(prev):
            if not finite(daily) or abs(float(daily)-(float(cur)-float(prev)))>1e-12:daily_failures.append((r.get('newsId'),h,daily,cur,prev))
        elif daily is not None:daily_failures.append((r.get('newsId'),h,'daily_AR_present_across_missing_CAR_gap'))
    for k in ('1','2','5'):
        if pba.get(k) is True:
            if not finite(pbr.get(k)) or (par.get(k) is not None and not finite(par.get(k))):pre_failures.append((r.get('newsId'),k,'invalid_pre_benchmark_or_AR'))
        elif pbr.get(k) is not None or par.get(k) is not None:pre_failures.append((r.get('newsId'),k,'raw_pre_return_relabelled_as_pre_AR'))
coverage=len(bench5)/max(1,len(matured5));policy=event.get('benchmarkPolicy') or {}
checks={
...
out={'version':'VMEWS-VNINDEX-BENCHMARK-GATE-12.1.0','generatedAt':datetime.now(timezone.utc).isoformat(),'status':'PASS' if all(checks.values()) else 'FAIL','benchmark':'VNINDEX','checks':checks,'indexAudit':idx,'cacheFallbackAudit':cache_detail if idx.get('route')=='LAST_GOOD_INDEX_CACHE' else None,'maturedH5Records':len(matured5),'benchmarkH5Available':len(bench5),'benchmarkH5Coverage':coverage,'semanticFailures':semantic_failures[:50],'targetAlignmentFailures':target_failures[:50],'preEventFailures':pre_failures[:50],'dailyARFailures':daily_failures[:50],'policy':'Event abnormal returns are VNINDEX-relative from event origin to the exact stock maturity date. Missing post/pre benchmark observations abstain; raw stock returns are never substituted or relabelled as abnormal returns. Audited VNINDEX cache is an accepted provider-outage route only when its integrity contract matches the training index audit.'}
