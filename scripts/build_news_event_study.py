...
def main(root='.'):
 ...
 for s in sorted(sent.get('symbols',{})):
  ...
  if latest:
   latestAssigned=next((x for x in allitems if x['symbol']==s and x.get('title')==latest.get('title')),None)
  eventMatched=groups['event'].get(str(latest.get('event'))) if latest else None;clusterId=latestAssigned.get('cluster') if latestAssigned else None;clusterMatched=groups['cluster'].get(str(clusterId)) if clusterId is not None else None;clusterMeta=clusterInfo.get(str(clusterId)) if clusterId is not None else None
  bysym[s]={'newsCount':cur.get('n',0),'coverageGrade':cur.get('coverageGrade','THIN'),'sentiment':cur.get('state'),'counts':cur.get('counts'),'eventStudy':agg(rows),'latestEvent':{'title':latest.get('title'),'event':latest.get('event'),'sourceClass':latest.get('sourceClass'),'label':latest.get('label'),'cluster':clusterId,'clusterTerms':(clusterMeta or {}).get('topTerms'),'historicalSameEvent':eventMatched,'historicalSameCluster':clusterMatched} if latest else None}
 ...
 out={'version':'VMEWS-NEWS-EVENT-STUDY-1.2.0','generatedAt':datetime.now(timezone.utc).isoformat(),'method':'Publication-time alignment in Vietnam; corporate-action-guarded closes; cross-sectional median market adjustment; pre-event AR(-5,-2,-1), post-event AR(+1..+5), T+2 confirmation; supervised sentiment plus TF-IDF/MiniBatchKMeans descriptive clusters','pointInTimeEligibleForForecast':False,'warning':'Retrospective news search can have availability/revision bias. Event-study, cluster and rumor statistics are descriptive context only and do not enter the numerical forecast.','events':len(mature),'symbolsWithPrice':sum(bool(x) for x in prices.values()),'clusters':clusterInfo,'groups':groups,'rumorStudy':rs,'clarificationStudy':cs,'symbols':bysym}
