...
entity=audit.get('entityFilter') or {};check('ENTITY gate version present',str(entity.get('version','')).startswith('VMEWS-ENTITY-GATE-12'),entity.get('version'));check('ENTITY gate evaluated >= 10000 source articles',int(entity.get('input') or 0)>=10000,entity);check('ENTITY gate rejected contaminated candidates',int(entity.get('rejected') or 0)>0,entity.get('rejected'));check('ENTITY retained coverage >= 55%',float(entity.get('acceptanceRate') or 0)>=.55,entity.get('acceptanceRate'));check('ENTITY ambiguous HCM audited','HCM' in (entity.get('perSymbol') or {}),(entity.get('perSymbol') or {}).get('HCM'));check('ENTITY ambiguous CDC audited','CDC' in (entity.get('perSymbol') or {}),(entity.get('perSymbol') or {}).get('CDC'))
...
phase={'version':'VMEWS-PHASE-GATES-12.1.0','generatedAt':datetime.now(timezone.utc).isoformat(),'tests':len(passes)+len(failures),'passed':len(passes),'failed':len(failures),'failures':failures,'phases':{k:not any(str(x['name']).startswith(prefix) for x in failures) for k,prefix in [('data','DATA'),('entity','ENTITY'),('event','EVENT'),('rumor','RUMOR'),('flow','FLOW'),('ml','MODEL'),('backtest','BACKTEST'),('dashboard','DASH')]}};phase['status']='PASS' if not failures else 'FAIL';(DATA/'phase-gates-v12.json').write_text(json.dumps(phase,ensure_ascii=False,indent=2,allow_nan=False),encoding='utf-8');print(json.dumps(phase,ensure_ascii=False,indent=2));raise SystemExit(1 if failures else 0)
