...
if(model.version!=='VMEWS-FORECAST-10.1.0'||model.promotion?.status!=='PASS')throw Error('model');
...
for(const h of ['3','5']){const z=model.horizons[h];if(z.status!=='PASS'||!z.gates.rankingApproved||!z.gates.directionApproved||!z.gates.alphaCalibrationApproved||!z.sealedAudit?.bootstrap?.ic95)throw Error('audit '+h);if((z.alphaCalibrationBuckets||[]).length<6||(z.directionCalibrationBuckets||[]).length<6)throw Error('calibration '+h)}
...
if(ev.version!=='VMEWS-NEWS-EVENT-STUDY-1.2.0'||ev.events<50||ev.pointInTimeEligibleForForecast!==false||!ev.rumorStudy?.preEvent?.['2'])throw Error('events');
const frt=ev.symbols?.FRT?.latestEvent;if(!frt||frt.cluster===undefined||frt.cluster===null||!frt.historicalSameCluster)throw Error('cluster evidence');
...
console.log(JSON.stringify({ok:true,model:model.version,symbols:model.universe.symbols,rows:model.universe.rows,featureCount:model.featureNames.length,absoluteScenario:model.promotion.absoluteScenario,T3:model.horizons['3'].sealedAudit,T5:model.horizons['5'].sealedAudit,FRTNews:news.coverage.FRT,eventStudy:ev.events,rumorStudy:ev.rumorStudy,FRTCluster:{cluster:frt.cluster,terms:frt.clusterTerms,n:frt.historicalSameCluster.n}}));
