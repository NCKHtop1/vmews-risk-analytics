...
if(model.universe?.symbols<50||model.universe?.rows<50000)throw Error('universe');
...
console.log(JSON.stringify({ok:true,model:model.version,featureCount:model.featureNames.length,T3:model.horizons['3'].sealedAudit,T5:model.horizons['5'].sealedAudit,FRTNews:news.coverage.FRT,eventStudy:ev.events,rumorStudy:ev.rumorStudy}));
