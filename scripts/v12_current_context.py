...
# Free VNStock guest quota is deliberately respected. 32 fundamental calls across the default
# eight-symbol research list remain below the rolling request limit instead of racing the API.
VNSTOCK_FUNDAMENTAL_INTERVAL=float(os.environ.get('V12_FUNDAMENTAL_INTERVAL','3.5'))
...
def previous_period_col(df,current):
    ...
    if cur in keys and keys.index(cur)>0:return uq[keys[keys.index(cur)-1]]
    return None
...
def row_map(df):
    ...
    try:
        for _,r in df.iterrows():out.append({str(k):serial(v) for k,v in r.to_dict().items()})
    except Exception:pass
    return out
...
def summarize_fundamental(symbol,fe):
    ...
    ratio,er=safe(lambda:fe.ratios(period='quarter'));income,ei=safe(lambda:fe.income_statement(period='quarter'));balance,eb=safe(lambda:fe.balance_sheet(period='quarter'));cash,ec=safe(lambda:fe.cash_flow(period='quarter'))
    rp=latest_period_col(ratio);ip=latest_period_col(income);bp=latest_period_col(balance);cp=latest_period_col(cash);rrows=row_map(ratio);irows=row_map(income);prev_i=previous_period_col(income,ip)
...
def typed_flow(rows,typ,asof):
    ...
    # CafeF foreign JSON is emitted in VND; the proprietary XLSX explicitly labels value columns
    # "Giá trị (tỷ VNĐ)". Normalize the latter to VND only for human-readable current context.
