...
ROOT=Path(os.environ.get('GITHUB_WORKSPACE','.'));MODEL='mnguyn11/phobert-stock-sentiment-PTDLW';VERSION='VMEWS-SENTIMENT-11.1.0'
...
def override(text,base):
    ...
    return base,'phobert'
def main():
    src=json.loads((ROOT/'data/news-history-v11.json').read_text(encoding='utf-8'));tok=AutoTokenizer.from_pretrained(MODEL,use_fast=False);mdl=AutoModelForSequenceClassification.from_pretrained(MODEL);mdl.eval();torch.set_num_threads(max(1,min(8,os.cpu_count() or 2)));id2={int(k):str(v).upper() for k,v in mdl.config.id2label.items()};out={'version':VERSION,'generatedAt':datetime.now(timezone.utc).isoformat(),'sourceVersion':src['version'],'model':MODEL,'symbols':{}};allrows=[]
    for s,a in src['symbols'].items():
        for x in a:allrows.append((s,x))
    labels={};batch=96
    for i in range(0,len(allrows),batch):
        part=allrows[i:i+batch];texts=[ViTokenizer.tokenize(str(x['title'])) for _,x in part];z=tok(texts,return_tensors='pt',padding=True,truncation=True,max_length=160)
        with torch.inference_mode():pr=torch.softmax(mdl(**z).logits,dim=-1)
        ...
        if i and i%(batch*40)==0:print(json.dumps({'sentimentRows':i,'total':len(allrows)}))
    for s,a in src['symbols'].items():
