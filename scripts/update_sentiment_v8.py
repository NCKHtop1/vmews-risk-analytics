...
def override(text,base):
 t=text.lower()
 if any(re.search(x,t) for x in NEG_RULE):return 'NEG','finance-rule'
 if any(re.search(x,t) for x in POS_RULE):return 'POS','finance-rule'
 if any(re.search(x,t) for x in NEU_RULE):return 'NEU','finance-rule'
 return base,'dl'
def relevant(sym,title):
 t=title.lower()
 if sym in SIBLING and any(re.search(x,t) for x in SIBLING[sym]):
  return bool(re.search(r'tập đoàn fpt|fpt corporation|cổ phiếu fpt|mã fpt',t))
 return True
def main(root='.'):
 p=Path(root,'data/research-news.json');src=json.loads(p.read_text(encoding='utf-8'));tok=AutoTokenizer.from_pretrained(MODEL,use_fast=False);mdl=AutoModelForSequenceClassification.from_pretrained(MODEL);mdl.eval();id2={int(k):str(v).upper() for k,v in mdl.config.id2label.items()};now=max([ts(x.get('published')) for a in src.get('symbols',{}).values() for x in a] or [datetime.now(timezone.utc).timestamp()]);out={'version':VERSION,'generatedAt':datetime.now(timezone.utc).isoformat(),'sourceGeneratedAt':src.get('generatedAt'),'model':MODEL,'method':'PhoBERT financial sentiment + finance-specific semantic overrides + recency/materiality/source/relevance weighting','symbols':{}}
 for sym,items in src.get('symbols',{}).items():
  ...
  for x in sorted(items,key=lambda z:ts(z.get('published')),reverse=True):
   ...
   if not title or k in seen or not relevant(sym,title):continue
  ...
  for x,(base,conf) in zip(clean,pred):
   lab,how=override(str(x.get('title') or ''),base);cnt[lab]+=1;age=max(0,(now-ts(x.get('published')))/86400);rec=math.exp(-age/35);w=max(.02,float(x.get('sourceQuality') or .6)*float(x.get('materiality') or .5)*float(x.get('relevance') or 1)*rec);val=1 if lab=='POS' else (-1 if lab=='NEG' else 0);ss+=w*val;sw+=w;scored.append({'title':x.get('title'),'published':x.get('published'),'publisher':x.get('publisher'),'event':x.get('event'),'label':lab,'confidence':conf,'method':how,'weight':w,'link':x.get('link')})
  mean=ss/sw if sw else 0.;out['symbols'][sym]={'available':True,'n':len(scored),'counts':cnt,'signed':mean,'riskScore':50-50*mean,'state':'TÍCH CỰC' if mean>.15 else ('TIÊU CỰC' if mean<-.15 else 'TRUNG TÍNH'),'top':sorted(scored,key=lambda x:x['weight'],reverse=True)[:8]}
 Path(root,'data/sentiment-v8.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps({'version':VERSION,'symbols':len(out['symbols']),'FPT':out['symbols'].get('FPT')},ensure_ascii=False,default=str))
