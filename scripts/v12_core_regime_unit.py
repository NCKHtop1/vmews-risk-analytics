...
dates = np.asarray([f'2026-01-{1 + i:02d}' for i in range(24)], dtype=object)
...
train = np.isin(D, dates[:16])
test = np.isin(D, dates[16:])
...
y2 = 0.020 * num_component + 0.020 * reg_component + rng.normal(scale=0.010, size=n)
expert_pred2 = {
    'NUMERICAL': num_component + rng.normal(scale=0.15, size=n),
    'REGIME': reg_component + rng.normal(scale=0.15, size=n),
}
