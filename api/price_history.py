...
def _normalize_df(df, symbol, provider):
    ...
    if time_col is None or close_col is None:
        raise RuntimeError(f'{symbol}: unexpected {provider} OHLCV columns {list(df.columns)}')
    raw = []
    ...
    if not raw:
        raise RuntimeError(f'{symbol}: {provider} OHLCV contained no usable rows')
...
def vnstock_equity_history(symbol, years=11):
    """Fallback daily OHLCV for Vietnamese equities using Vnstock free APIs.

    Try Unified Market with an explicit large candle count first. If the free UI
    still truncates the range, try the legacy Quote.history interface with VCI
    and then KBS. Prices returned in thousand-VND are normalized back to VND.
    """
    ...
    try:
        ...
        rows, scale = _normalize_df(df, symbol, 'Vnstock Unified Market')
        if len(rows) >= 240:
            return rows, {
                'source': 'Vnstock',
        ...
        errors.append(f'Unified Market only {len(rows)} rows')
    except Exception as e:
        errors.append(f'Unified Market: {e}')
    ...
    try:
        ...
        for source in ('VCI', 'KBS'):
            try:
                ...
                rows, scale = _normalize_df(df, symbol, f'Vnstock {source} Quote.history')
                if len(rows) >= 240:
                    return rows, {
                        'source': 'Vnstock',
                        'provider': f'{source} Quote.history',
                        'symbol': symbol,
                        'rows': len(rows),
                        'unitNormalization': 'x1000 to VND' if scale == 1000.0 else 'already VND',
                        'ok': True,
        ...
        errors.append(f'Legacy Vnstock import: {e}')

    raise RuntimeError(f'{symbol}: no Vnstock long-history route succeeded; ' + ' | '.join(errors))
