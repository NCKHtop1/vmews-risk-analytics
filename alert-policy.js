...
const POLICY_URL=new globalThis.URL('./data/alert-policy.json',location.href).href;
let value=FALLBACK;
const ready=fetch(`${POLICY_URL}?t=${Date.now()}`,{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject(Error(`Alert policy HTTP ${r.status}`))).then(p=>{value=p;return p}).catch(()=>FALLBACK);
window.VMEWSAlertPolicy={get value(){return value},ready,url:POLICY_URL,fallback:FALLBACK};
})();
